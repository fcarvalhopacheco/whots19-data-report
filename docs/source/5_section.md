...
WHOTS-19 deployment. For all these sensors, the mean difference from the
...
panel), and 7215 (bottom panel), during the WHOTS-19 deployment. The light blue
...
VMCM and the 7-m MicroCAT temperatures during WHOTS-19, after adding a 0.0259°C 
...
21, 2020 (see {ref}`/3_section.md#description-of-whots-19-mooring`). For 
...
```{figure} figures/microcats/w19tcompare_33.png
:height: 1000px
:align: center
:name: figure5.4
...
conductivity sensors during WHOTS-19 are shown in {numref}`figure5.8`
...
between MicroCATs #12241 and #11391 during WHOTS-19. The blue (red) lines are
...
```{figure} figures/microcats/w19mic_corr8.jpg
:height: 1000px
:align: center
:name: figure5.15

Conductivity sensor correction for MicroCAT at 4713 meters during WHOTS-19
```
...
```{figure} figures/adcp_moored/adcp_whot19cmpserr_sn4891.png
:height: 600px
:align: center
:name: figure5.16
...
```{figure} figures/adcp_moored/adcp_whot19cmpserr_sn1825.png
:height: 600px
:align: center
:name: figure5.17
...
detailed in {ref}`/appendices.md#whots-19-300-khz-serial-4891`, and
{ref}`/appendices.md#whots-19-600-khz-serial-1825`. The salient differences for
...
```{table} ADCP record times (UTC mm/dd/yyyy, hh:mm:ss) during WHOTS-19 deployment
:class: sd-m-auto
:align: center
:name: table-21
...
WHOTS 
location. A heading bias of 9.54° was entered in the setup of the WHOTS-19 
ADCP’s.
...
```{figure} figures/adcp_moored/wh19_CTD_sv_profile.png
:height: 800px
:align: center
:name: figure5.20

Sound speed profile (top panel) during the deployment of the WHOTS-19 mooring
from 2 dbar CTD data taken during regular HOT cruises and CTD profiles taken
during the WHOTS-19 and -20) deployment cruises (individual casts marked with a
...
4. An indication of data quality for each ensemble is given by the “percent
   ...
   {ref}`/appendices.md#whots-19-300-khz-serial-4891`). Data were flagged when
